...
*/
var debug = require('debug')('jggj:sceptre-caller');

var sceptre = require('./sceptre');
...
function callSceptre(input, done) {
  ...
  */
  ...
  var query = sceptre.fromJSON(input);
  debug('To SCEPTRE: ' + query);

  var options = {
    url: 'http://localhost:9080/PCW/service/proxy',
    method: 'POST',
    body: query
  };
  ...
  request(options, function (err, res, body) {
    ...
    */
    ...
    debug("callSceptre calling success callback with data of type: " + format);
    done(null, output);
  });
}
